...
*//*        //测试--统计对应的字符出现的次数
        ...
        //测试--创建对应的赫夫曼树
        Node hfroot = creatHt(nodes);
        System.out.println("前序遍历：");
        hfroot.preOrder();

        //测试--创建对应的赫夫曼编码
        Map<Byte, String> hfmcodes = getCodes(hfroot);
        System.out.println(hfmcodes);

        //测试--生成哈夫曼编码对应的字符数组
        byte[] zipcodes = zip(contentBytes, hfmcodes);
...
//        System.out.println("测试StringBuilder----"+stringBuilder.toString());
    ...
    private static Map<Byte, String> getCodes(Node root) {
        ...
        //处理root的左子树
        getCodes(root.left, "0", stbr);
        //处理root的右子树
        getCodes(root.right, "1", stbr);
        return huffmanCodes;
    }
     ...
     *
     * @param bytes 接收字节数组
     * @return 返回的就是 nodes 形式   [Node[date=97 ,weight = 5], Node[]date=32,weight = 9]......],
     */
